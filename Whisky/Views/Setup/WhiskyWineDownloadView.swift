...
struct WhiskyWineDownloadView: View {
            ...
            VStack {
                ...
                if let errorMessage {
                    VStack(spacing: 10) {
                        ...
                        Button("Choose Local Libraries.tar.gz...") {
                            chooseLocalArchive()
                        }
                    }
                    .padding(.horizontal)
    ...
    func startDownload() {
        ...
        let url = WhiskyWineInstaller.archiveURL
            ...
            Task { @MainActor in
                do {
                    ...
                    tarLocation = try WhiskyWineInstaller.persistDownloadedArchive(
                        at: url,
                        response: response,
                        sourceURL: WhiskyWineInstaller.archiveURL
                    )
                    proceed()
                } catch {
    ...
    func updateProgress(_ task: URLSessionDownloadTask) {
